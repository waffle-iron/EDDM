...
Partial Class SavedLists
    ...
    Protected Sub Page_Init1(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        If Not (SiteDetails.UseRibbonBanners) Then
            ...
            PageHeader.simpleHeader = "Your Saved Lists"
        Else
            PageHeader.headerType = "partial"
            PageHeader.mainHeader = "Saved Lists"
            PageHeader.subHeader = "Your Saved Lists"
        End If
    ...
    Public Class SavedList
        ...
        Public Property TotalDeliveries() As Integer
            Get
                Return _Quantity
            End Get
            Set(ByVal value As Integer)
    ...
    Public Shared Function RetrieveDescription(DistributionID As Integer, UselectID As Integer) As String

        If UselectID = 5 Then
            Return "Uploaded List"
        ElseIf UselectID = 6 Then
            Return "Generated List"
        Else
            Return AddressedListUtility.GetSavedFilters(DistributionID)
        End If
    ...
    Public Shared Function GetDataFilters(DistributionID As Integer) As String
        Return AddressedListUtility.GetSavedFilters(DistributionID)
    End Function


    Public Shared Function RetrieveSavedListsForCustomerID(customerID As Integer) As List(Of SavedList)
        Dim lstSavedList As New List(Of SavedList)
        Try
            Using oConnA As New SqlConnection(ConfigurationManager.ConnectionStrings("TaradelWLConnectionString").ConnectionString)
                Dim sSql As String = "SELECT * FROM pnd_CustomerDistribution WHERE CustomerID = " + customerID.ToString() + " and USelectId IN (5,6) and deleted = 0 ORDER BY CreatedDate DESC"
                ...
                Using oCmdA As New SqlCommand(sSql, oConnA)
                    ...
                    Using RDR As SqlDataReader = oCmdA.ExecuteReader()
                        While (RDR.Read())
                            Dim saveList As New SavedList()
                            saveList.CreatedDate = TaradelReceiptUtility.OrderCalculator.ConvertToDate(RDR("CreatedDate"))
                            saveList.Name = RDR("Name")
                            saveList.ReferenceID = RDR("ReferenceID").ToString()
                            saveList.DistributionId = TaradelReceiptUtility.OrderCalculator.ConvertToIntegerOrZero(RDR("DistributionId"))
                            saveList.TotalDeliveries = TaradelReceiptUtility.OrderCalculator.ConvertToIntegerOrZero(RDR("TotalDeliveries"))
                            saveList.USelectID = TaradelReceiptUtility.OrderCalculator.ConvertToIntegerOrZero(RDR("USelectId"))
                            If (saveList.TotalDeliveries > 0) Then
                                lstSavedList.Add(saveList)
                            End If
                        End While
                    End Using
        ...
        Catch ex As Exception
            Dim exSaveList As New SavedList()
            exSaveList.CreatedDate = DateTime.Now
            exSaveList.DistributionId = 0
            exSaveList.Name = "error"
            exSaveList.TotalDeliveries = 0
            exSaveList.Name = ex.ToString()
            lstSavedList.Add(exSaveList)
        End Try

        If (lstSavedList.Count > 0) Then
            Try
                Using oMapConn As New SqlConnection(ConfigurationManager.ConnectionStrings("MapServerConn").ConnectionString)
                    Dim sbSQL As StringBuilder = New StringBuilder()
                    sbSQL.Append("SELECT * FROM SavedListSelection WHERE ReferenceId IN (")
                    For Each savedList As SavedList In lstSavedList
                        sbSQL.Append("'")
                        sbSQL.Append(savedList.ReferenceID)
                        sbSQL.Append("',")
                    Next

                    sbSQL.Length = sbSQL.Length - 1 'remove the comma
                    sbSQL.Append(")")


                    Using oCmdA As New SqlCommand(sbSQL.ToString(), oMapConn)
                        oCmdA.CommandType = CommandType.Text
                        oMapConn.Open()
                        Using RDR As SqlDataReader = oCmdA.ExecuteReader()
                            While (RDR.Read())
                                For Each savedList As SavedList In lstSavedList
                                    If (savedList.ReferenceID = RDR("ReferenceID").ToString()) Then
                                        savedList.Description = RDR("Selection").ToString()
                                    End If
                                Next
    ...
    Protected Sub BindList()
        'Step 1 ' Retrieve the Taradel Reference IDs
        Dim lstSavedList As New List(Of SavedList)
        lstSavedList = RetrieveSavedListsForCustomerID(Me.CustomerId)
        lvSelects.DataSource = lstSavedList
        lvSelects.DataBind()


        'Dim sql As String = "SELECT * FROM pnd_CustomerDistribution WHERE CustomerID = 95469 and USelectId IN (5,6) And Deleted = 0"
        'DistributionID | CustomerID | Name | CreatedDate | ReferenceId | TotalDeliveries | USelectId | Deleted

        'Step 2 Build List of ReferenceIDs to query ValMap-Dev
    ...
    Protected Sub lvSelects_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ListViewItemEventArgs) Handles lvSelects.ItemDataBound
        If e.Item.ItemType = ListViewItemType.DataItem Then
            ...
            Dim iSelectId As Integer = 0  'oSelect.USelectMethod.USelectId

            'find lDescription
            Dim lDescription As Literal = DirectCast(e.Item.FindControl("lDescription"), Literal)
            Dim hfDistributionId As HiddenField = DirectCast(e.Item.FindControl("hfDistributionId"), HiddenField)
            Dim DistributionId As Integer = Integer.Parse(hfDistributionId.Value)
            Dim hypViewFilters As HyperLink = DirectCast(e.Item.FindControl("hypViewFilters"), HyperLink)

            If (lDescription.Text = "Uploaded List") Then
                'hide Edit Button
                Dim lnkEdit As LinkButton = DirectCast(e.Item.FindControl("lnkEdit"), LinkButton)
                lnkEdit.Visible = False
                iSelectId = 5 'uploaded list
            Else
                Dim lnkEdit As LinkButton = DirectCast(e.Item.FindControl("lnkEdit"), LinkButton)
                lnkEdit.Visible = False 'BuildList not smart enough yet 9/30/15
                iSelectId = 6 'build list
                hypViewFilters.Visible = True
            End If
            ...
            Dim oSelect As Taradel.CustomerDistribution = DirectCast(oDist, Taradel.CustomerDistribution)
            ...
            If iSelectId = 5 Or iSelectId = 6 Then
    ...
    Protected Sub OpenListForEdit(sender As Object, e As System.EventArgs)
    ...
    '                                    If chkPasteBusiness.Checked Then
    '                                        oArea.Business = oRdr.GetInt32(3)
    '                                    Else
    '                                        oArea.Business = 0
    '                                    End If
    '                                    oArea.Residential = oRdr.GetInt32(2)
    '                                    If chkPastePOBoxes.Checked Then
    ...
    Private Function DeleteDistribution(DistributionID As Integer) As Boolean
        Dim returnThis As Boolean = False
        Dim resultCount As Integer = 0
        Using oConn As New SqlConnection(ConfigurationManager.ConnectionStrings("TaradelWLConnectionString").ConnectionString)
            oConn.Open()
            Dim sql As String = "UPDATE pnd_CustomerDistribution SET Deleted = 1 WHERE DistributionId = " & DistributionID
            Using oCmd As New SqlCommand(sql, oConn)
                resultCount = oCmd.ExecuteNonQuery()
            End Using
            oConn.Close()
        End Using
        returnThis = (resultCount > 0)

        Return returnThis
    End Function


    Protected Sub DeleteList(sender As Object, e As System.EventArgs)
        Dim oLnk As LinkButton = DirectCast(sender, LinkButton)
        ...
        If Integer.TryParse(sDistId, iDistId) Then
            Dim sMsg As String = ""
            Dim bDeleted As Boolean = DeleteDistribution(iDistId) 'Taradel.CustomerDistributions.Delete(iDistId, sMsg)

            If bDeleted Then
                sMsg = "Distribution List " & iDistId.ToString() & " Deleted " & DateTime.Now.ToShortDateString & " " & DateTime.Now.ToShortTimeString() & "."
                ShowSuccess(sMsg, "myprojects")
                BindList()
            Else
    ...
    Protected Sub btnUploadNewList_Click(sender As Object, e As EventArgs) Handles btnUploadNewList.Click
        Response.Redirect("~/Addressed/Step1-UploadYourList.aspx")
    End Sub


    Protected Sub btnGenerateNewList_Click(sender As Object, e As EventArgs) Handles btnGenerateNewList.Click
